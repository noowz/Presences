...
presence.on('UpdateData', async () => {
  ...
  const presenceData: MediaPresenceData = {
    name: Name,
    type: ActivityType.Playing as ActivityType.Watching,
    startTimestamp: GameState.startedAt,
    largeImageText: gameName || undefined,
    largeImageKey: await fetchCharacterFace().then(url => url || Logo),
    smallImageKey: await fetchBadge().then(url => url || undefined),
    details: gameName || 'Choosing a game...',
    state: gameName ? gameLocation || 'Disconnected' : undefined,
    buttons: gameName
...
async function fetchWithResizePixelatedImage(
  ...
  return new Promise((resolve, reject) => {
    ...
    // @ts-expect-error canvas.toBlob is not typed
