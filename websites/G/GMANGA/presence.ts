...
const AVERAGE_READING_TIME = 12000
const searchInput = document.querySelector<HTMLInputElement>('#quickSearch')
const startTimestamp: number = Math.floor(Date.now() / 1000)
function router({ path, data }: { path: string, data: PresenceData }): Route {
  const routes: Route[] = [
    ...
    },
    {
      path: /^\/mangas\/\d+\/(.*)\/[0-9.]+\/(.*)$/,
      run: () => {
        const endTimestamp = document.querySelectorAll('[id^="page_"]').length
      ...
      state: () => document.querySelector('.white-link')?.textContent ?? undefined,
      details: () =>
        `Chapter: ${
...
if (searchInput) {
  searchInput.addEventListener('input', () => {
    ...
    searchData.state = `Searching: ${searchInput.textContent}`
  })
}
...
presence.on('UpdateData', async () => {
  ...
  if (route.run)
    presenceData = route.run()
  if (route.details)
    presenceData.details = route.details()
  if (route.buttons && showButtons)
    presenceData.buttons = route.buttons() as [ButtonData, (ButtonData | undefined)?]
  if (route.largeImageKey)
  ...
  if (searchData.smallImageKey || route.smallImageKey) {
    presenceData.smallImageKey = searchData.smallImageKey || route.smallImageKey?.()
  }

  if (searchData.smallImageText || route.smallImageText) {
    presenceData.smallImageText = searchData.smallImageText || route.smallImageText?.()
  }
