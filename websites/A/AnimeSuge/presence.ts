...
presence.on(
  'iFrameData',
  async (data: unknown) => {
    ({ currentTime, timeEnd, paused } = data as { currentTime: number, timeEnd: number, paused: boolean })
  },
)
...
presence.on('UpdateData', async () => {
  ...
  const epNumber = pathname.slice(pathname.length - 5).replace(/^\D+/g, '')
  ...
  if (pathname.includes('/anime/')) {
    ...
    if (epNumber === '')
      presenceData.state = 'Full episode'
    else presenceData.state = `Episode ${epNumber}`

    if (!paused) {
      presenceData.smallImageKey = Assets.Play;
      [presenceData.startTimestamp, presenceData.endTimestamp] = presence.getTimestamps(currentTime, timeEnd)
    }
    else {
